...
def home(request: HttpRequest) -> HttpResponse:
    ...
    return render(request, "bootstrap_home.html", {"products": products, "page_title": "Josmee Online Shopping"})

def products_list(request: HttpRequest) -> HttpResponse:
    products = Product.objects.all().order_by("-id")
    return render(request, "products_list.html", {"products": products, "page_title": "All Products - Josmee Online Shopping"})

def product_detail(request: HttpRequest, pk: int) -> HttpResponse:
    product = get_object_or_404(Product, pk=pk)
    return render(request, "product_detail.html", {"product": product, "page_title": f"{product.name} - Josmee Online Shopping"})
...
def cart_view(request: HttpRequest) -> HttpResponse:
    ...
    return render(request, "cart.html", {"items": items, "total": total, "page_title": "Your Cart - Josmee Online Shopping"})
...
def checkout_view(request: HttpRequest) -> HttpResponse:
    ...
    if request.method == "POST":
        ...
        return render(request, "checkout_success.html", {"page_title": "Order Placed - Josmee Online Shopping"})
    # compute totals for display
    total = sum(Decimal(item["price"]) * int(item["qty"]) for item in cart.values())
    return render(request, "checkout.html", {"total": total, "page_title": "Checkout - Josmee Online Shopping"})
